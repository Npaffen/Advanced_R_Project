### Language Processing (legacy code for creating the dictionary from scratch)
...
for(i in files){
  ...
  if(file.exists(paste0(wdir,"/output/dictionary.rds")) & !OVERWRITE ){
    cat("found existing dictionary.rds, translating and appending new words \n")
  ...
  if(RUN_API){ # only run once! character limit: 1,000,000/day, 10,000,000 month
    cat("starting dictionary translation with Yandex API \n")
