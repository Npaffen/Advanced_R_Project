...
paper_length <- read_html(remDr$getPageSource()[[1]]) %>% 
  ...
  articles_fun<- function(paper_length){
  remDr$navigate(str_c("http://data.people.com.cn.s894ibwr0870.erf.sbb.spk-berlin.de/rmrb",date[1], paper_length, sep="/")) 
  read_html(remDr$getPageSource()[[1]]) %>%
  html_nodes("li h3 a") %>%
  html_attr("href") %>%
  gsub('(/[a-z]+/\\d+/\\d+/)', replacement ="", x = . ) %>%
  grep('([a-z0-9]{32})' , value = T, x = .)
  gsub('^', replacement = str_c(paper_length,"/",sep = ""), x = .)
  }
  articles <- map(paper_length,~articles_fun(.x))
  articles<- map(paper_length[1], 
                 ~(str_c("http://data.people.com.cn.s894ibwr0870.erf.sbb.spk-berlin.de/rmrb",date[1], .x, sep="/")))%>%
    map( ~remDr$navigate(.x) %>%
            read_html(x = remDr$getPageSource()[[1]]))
  
map(articles, ~remDr$navigate(str_c("http://data.people.com.cn.s894ibwr0870.erf.sbb.spk-berlin.de/rmrb",
                                    date,paper_length,articles, sep = "/"))
    $read_html(remDr$getPageSource()[[1]]) %>%
      map(.x = all.nodes, ~ . %>%
html_nodes(.x) %>%
  html_text() %>%
